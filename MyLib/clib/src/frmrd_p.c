/*                      - FRMRD_P.C -
   ...
   Version: 1.04 [IPEO] (from frmrd 3.33G)
      */
...
static int get_num_and_scan_P (PGM_P *ptr, int *wptr)
  {
    int n = 0;
...
#ifndef REDUCED_SUPPORT
static char get_valid_char (char ch, char standard_compare, PGM_P str) 
  {
    do
      {
        if (ch == *str)
...
int _formatted_read_P (const char **line, PGM_P *format, va_list ap)
  {
...
/*======================================================================*/
        ...
        if (isdigit (c))
          {
            i = MAX_FIELD_ALLOWED;
	    field_width = get_num_and_scan_P (format,&i);
            c = **format;
          }
        else
          {
                  ...
                  if (exp_sign)
                    {
                      ...
                      if (sizeof(float) != sizeof(double) &&
		          !(flags & LONG_FLAG) && fvalue < FLT_MIN)
                        {
                          goto BAD_CONVERSION;
                        }
                    }
